...
public class Textbox2 {
	...
	public Textbox2(BitmapFont font,float tilesize){
		ready = false;
		x = 0;
		y = 0;
		border = 8;
	...
	public void open(GameState mode){
		if(mode == GameState.COMBAT || mode == GameState.MENU){
			...
			if(x == 32 && y > Gdx.graphics.getHeight()/2+32){
	...
	public void close(){
		if(currentState == GameState.COMBAT || currentState == GameState.MENU){
			...
			if(y >= Gdx.graphics.getHeight()*3/4-8 && x < Gdx.graphics.getWidth()/8-16){
				x += (Gdx.graphics.getWidth()/8-16-32)*Gdx.graphics.getDeltaTime()*3/20;
			}
			if(y >= Gdx.graphics.getHeight()*3/4-8 && x >= Gdx.graphics.getWidth()/8-16){
				x = 0;
